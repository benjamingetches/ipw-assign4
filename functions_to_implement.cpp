...
//same idea, but using while loop to find all substr
std::string RemoveAllSubstrings(std::string s1, std::string s2){
	if(s2.empty()){return s1;}
	size_t pos = s1.find(s2); //size_t needed as npos returns a sizet also. uses find to do lifting. 
...
std::string RemoveFirstSubstring(std::string s1, std::string s2){
	...
	if(pos != std::string::npos){
		s1.erase(pos,s2.length()); //using method to erase the len of s2 from s1
	}
	return s1;
}
...
// Sums all numbers in a vector and returns the resulting value
//ardouble Sum(std::vector<double> nums);
